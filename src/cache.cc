...
extern std::map<uint32_t, uint32_t> map_cnt; // modify
// uint32_t access_cnt;
...
void CACHE::handle_fill()
{
  while (writes_available_this_cycle > 0) {
    ...
    /*   access_cnt++;
       if(access_cnt == 100000)
       {
         for(auto iter = map_cnt.begin(); iter != map_cnt.end(); iter++)
           if (iter->second > 2)
   ...
   */
    ...
    if (NAME.compare("LLC") == 0) {
      auto idex = map_cnt.find(fill_mshr->v_address);
      if (idex == map_cnt.end())
        map_cnt.insert(std::make_pair(fill_mshr->v_address, 1));
      else
        idex->second++;
    }
  ...
  while (writes_available_this_cycle > 0) {
    ...
    */
    ...
    } else // MISS
    {
      ...
      // modify
      if (NAME.compare("LLC") == 0) {
        auto idex = map_cnt.find(handle_pkt.v_address);
        if (idex == map_cnt.end())
          map_cnt.insert(std::make_pair(handle_pkt.v_address, 1));
        else
          idex->second++;
      }
    }
